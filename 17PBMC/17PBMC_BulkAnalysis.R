...
pseudobulkobj=readRDS("~/Project_PBMCage/GSE213516_17PBMC/17PBMC_annotated_AssignedOnly_Pseudobulk_rough.rds")
...
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Rough.csv.gz", sep="\t")
...
pseudobulkobj=readRDS("~/Project_PBMCage/GSE213516_17PBMC/17PBMC_annotated_AssignedOnly_Pseudobulk_inter.rds")
all_genes=rownames(pseudobulkobj)
...
Cor_analysis_DF=data.table::rbindlist(Cor_analysis_DF_list)
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Inter.csv.gz", sep="\t")
...
pseudobulkobj=readRDS("~/Project_PBMCage/GSE213516_17PBMC/17PBMC_annotated_AssignedOnly_Pseudobulk_detailed.rds")
all_genes=rownames(pseudobulkobj)
...
Cor_analysis_DF=data.table::rbindlist(Cor_analysis_DF_list)
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Detailed.csv.gz", sep="\t")
...
### At the rough level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Rough.csv.gz", sep="\t")
...
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Rough.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()

### At the inter level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Inter.csv.gz", sep="\t")
all_celltypes=names(table(Cor_analysis_DF$celltypes))
upset_plot=list()
for (i in 1:length(all_celltypes)) {
  Subset_=subset(Cor_analysis_DF, celltypes==all_celltypes[i])
...
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Inter.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()

### At the detailed level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/GSE213516_17PBMC/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Detailed.csv.gz", sep="\t")
all_celltypes=names(table(Cor_analysis_DF$celltypes))
upset_plot=list()
for (i in 1:length(all_celltypes)) {
  Subset_=subset(Cor_analysis_DF, celltypes==all_celltypes[i])
...
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Detailed.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()
...
df_f=Cor_analysis_DF %>%
  ...
  select(p_val, avg_log2FC, p_val_adj, cluster, gene)
selected_genes_pos=df_f %>% mutate(ranks=rownames(.)) %>% group_by(cluster) %>% 
  subset(avg_log2FC>0) %>%
...
selected_genes_neg=df_f %>% mutate(ranks=rownames(.)) %>% group_by(cluster) %>% 
  subset(avg_log2FC<0) %>%
  slice_min(avg_log2FC, n=3) %>% 
  sample_n(size=3) %>% # because a lot of genes have the same 1.0 or -1.0 rho, we have to randomly pick 5 out of them for marking
  ungroup() %>% select(ranks) %>% unlist() %>% unname()
df_f=df_f %>% mutate(mark=ifelse(rownames(.) %in% c(selected_genes_pos, selected_genes_neg), gene, NA))
source("~/Rscripts/jjVolcano2.R")
plot_f=
...
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Rough.pdf", width=length(unique(df_all$cluster)), height=5)
...
plot_m=
  ...
  labs(x=NULL, y=expression(Spearman~rho), title="<males>")
# save all/females/males
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Inter_CD8T.pdf", width=5, height=5)
plot(plot_); plot(plot_f); plot(plot_m)
dev.off()
...
plot_m=
  ...
  labs(x=NULL, y=expression(Spearman~rho), title="<males>")
# save all/females/males
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Detailed_CD8T.pdf", width=10, height=5)
plot(plot_); plot(plot_f); plot(plot_m)
dev.off()
...
for (i in 1:length(celltypes_all)) {
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_pos.pdf")))
...
for (i in 1:length(celltypes_all)) {
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_CD8T.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_pos.pdf")))
...
for (i in 1:length(celltypes_all)) {
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_CD8T.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_pos.pdf")))
...
pdf("~/Project_PBMCage/Plots/17PBMC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Rough_MarkedwKnownGenes.pdf", height=5, width=6)
