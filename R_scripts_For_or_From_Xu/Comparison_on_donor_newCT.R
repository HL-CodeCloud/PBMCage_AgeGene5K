...
saveRDS(gene_list, "~/Project_PBMCage/For_or_From_Xu/Results/NEWCT_Compare.to.Published_Genesets_GenesetsToCompare.rds")
...
gene_list=readRDS("~/Project_PBMCage/For_or_From_Xu/Results/NEWCT_Compare.to.Published_Genesets_GenesetsToCompare.rds")[c(1:5,13)]
...
saveRDS(RSQs_all, "~/Project_PBMCage/Tempt_RDS/NEWCT_Xu_5000Genes_prediction.MAE.analysis.rds")

### Rearrange the results
RSQs_all=readRDS("~/Project_PBMCage/Tempt_RDS/NEWCT_Xu_5000Genes_prediction.MAE.analysis.rds")
...
levels(RSQs_DF_long$geneset)=c("de Magalhães et al. (2009)","GenAge Build 21","Dørum et al. (2024)","Yang et al. (2015)","Chuffa et al. (2022)",
                               ...
                               "AgeGene5K")
write.table(RSQs_DF_long, "~/Project_PBMCage/For_or_From_Xu/Results/NEWCT_Compare.to.PublishedGenesets_PredictParameters_inPBMCage_perDonorID.txt", sep="\t")
unlink("~/Project_PBMCage/Tempt_RDS/NEWCT_Xu_5000Genes_prediction.MAE.analysis.rds")

### Plot the R-squared
unique(RSQs_DF_long$geneset) # check
RSQs_DF_long=read.delim("~/Project_PBMCage/For_or_From_Xu/Results/NEWCT_Compare.to.PublishedGenesets_PredictParameters_inPBMCage_perDonorID.txt", sep="\t")
...
pdf("~/Project_PBMCage/For_or_From_Xu/Xu_Plots/NEWCT_Compare.to.PublishedGenesets_PredictParameters_inPBMCage_donor.pdf", width=10, height=8)
plot(plot_)
dev.off()
...
plot_=
  ...
  ggsci::scale_color_d3("category20c") +
  theme_classic() +
  labs(x="geneN", y="parameter", title="Combination of GSE213516, GSE158055, SC cohort") +
  theme(axis.text.x=element_text(size=8),
        axis.text.y=element_text(size=8),
        axis.title.x=element_text(size=9),
        axis.title.y=element_text(size=9),
...
pdf("~/Project_PBMCage/For_or_From_Xu/Xu_Plots/Compare.to.PublishedGenesets_PredictParameters_inCombinedVeri_donor.pdf", width=10, height=8)
plot(plot_)
dev.off()
...
pdf("~/Project_PBMCage/For_or_From_Xu/Xu_Plots/Compare.to.PublishedGenesets_PredictParameters_inBulkRNA.pdf", width=10, height=8)
plot(plot_)
dev.off()
...
### Make the pseudobulk on donor_id
pseudobulkobj=readRDS("~/Project_PBMCage/Immunity/all_pbmcs/Pseudobulk_on_TubeID.rds")
pseudobulkobj_donor=AggregateExpression(pseudobulkobj, assays="RNA", return.seurat=T, group.by=c("age","sex","donor_id"))
pseudobulkobj_donor$age=as.numeric(unlist(lapply(strsplit(colnames(pseudobulkobj_donor), split="_"), function(x) x[1])))
pseudobulkobj_donor$sex=unlist(lapply(strsplit(colnames(pseudobulkobj_donor), split="_"), function(x) x[2]))
pseudobulkobj_donor$donor_id=unlist(lapply(strsplit(colnames(pseudobulkobj_donor), split="_"), function(x) x[3]))
...
pdf("~/Project_PBMCage/For_or_From_Xu/Xu_Plots/Compare.to.PublishedGenesets_PredictParameters_inImmunity_donor.pdf", width=10, height=8)
plot(plot_)
dev.off()
