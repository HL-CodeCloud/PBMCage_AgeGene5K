...
pseudobulkobj=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_rough.rds")
...
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Rough.csv.gz", sep="\t")
...
pseudobulkobj=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_inter.rds")
all_genes=rownames(pseudobulkobj)
...
Cor_analysis_DF=data.table::rbindlist(Cor_analysis_DF_list)
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Inter.csv.gz", sep="\t")
...
pseudobulkobj=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_detailed.rds")
all_genes=rownames(pseudobulkobj)
...
Cor_analysis_DF=data.table::rbindlist(Cor_analysis_DF_list)
data.table::fwrite(Cor_analysis_DF, "~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Detailed.csv.gz", sep="\t")
...
### At the rough level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Rough.csv.gz", sep="\t")
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Rough.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()

### At the inter level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Inter.csv.gz", sep="\t")
all_celltypes=names(table(Cor_analysis_DF$celltypes))
upset_plot=list()
for (i in 1:length(all_celltypes)) {
  Subset_=subset(Cor_analysis_DF, celltypes==all_celltypes[i])
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Inter.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()

### At the detailed level
Cor_analysis_DF=data.table::fread("~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/Bulkanalysis_RNAexpr_Correlation_wAges_Detailed.csv.gz", sep="\t")
all_celltypes=names(table(Cor_analysis_DF$celltypes))
upset_plot=list()
for (i in 1:length(all_celltypes)) {
  Subset_=subset(Cor_analysis_DF, celltypes==all_celltypes[i])
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_UpSet_Detailed.pdf", width=5, height=4)
for(i in 1:length(upset_plot)) plot(upset_plot[[i]])
dev.off()
...
# # females (! since no sig. genes in any of the cluster, discard)
...
df_m=Cor_analysis_DF %>%
  ...
  select(p_val, avg_log2FC, p_val_adj, cluster, gene)
selected_genes_pos=df_m %>% mutate(ranks=rownames(.)) %>% group_by(cluster) %>% 
  subset(avg_log2FC>0) %>%
  slice_max(avg_log2FC, n=3) %>% 
  sample_n(size=3) %>% # because a lot of genes have the same 1.0 or -1.0 rho, we have to randomly pick 5 out of them for marking
  ungroup() %>% select(ranks) %>% unlist() %>% unname()
selected_genes_neg=df_m %>% mutate(ranks=rownames(.)) %>% group_by(cluster) %>% 
  subset(avg_log2FC<0) %>%
  slice_min(avg_log2FC, n=3) %>% 
  sample_n(size=3) %>% # because a lot of genes have the same 1.0 or -1.0 rho, we have to randomly pick 5 out of them for marking
  ungroup() %>% select(ranks) %>% unlist() %>% unname()
df_m=df_m %>% mutate(mark=ifelse(rownames(.) %in% c(selected_genes_pos, selected_genes_neg), gene, NA))
source("~/Rscripts/jjVolcano2.R")
plot_m=
  jjVolcano2(diffData=df_m,
             ...
             fontface="italic",
             topGeneN=0) +
  ggrepel::geom_text_repel(aes(label=mark), max.overlaps=50, size=3) + # because a lot of genes have the same 1.0 or -1.0 rho, we have to randomly pick 5 out of them for marking
  guides(color=guide_legend(size=3)) +
  theme(title=element_text(size=10),
        legend.text=element_text(size=12),
        legend.position="none",
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Rough.pdf", width=length(unique(df_all$cluster)), height=5)
plot(plot_)
# plot(plot_f)
plot(plot_m)
dev.off()
...
plot_m=
  ...
  labs(x=NULL, y=expression(Spearman~rho), title="<males>")
# save all/females/males
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Inter_CD8T.pdf", width=5, height=5)
plot(plot_)
# plot(plot_f)
plot(plot_m)
dev.off()
...
plot_m=
  ...
  labs(x=NULL, y=expression(Spearman~rho), title="<males>")
# save all/females/males
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Detailed_CD8T.pdf", width=20, height=5)
plot(plot_)
# plot(plot_f)
plot(plot_m)
dev.off()
...
for (i in 1:length(celltypes_all)) {
  ...
  Cor_df_neg=Cor_analysis_DF %>%
    subset(celltypes==celltypes_all[i] & rho_pval<0.05 & rho<0 & analysis=="all") %>% # mix both sex because some ages have only 1 female
    slice_min(rho, n=100) %>%
  ...
  expr_data$sex=fct_relevel(expr_data$sex, "F","M")
  # plot
  cor_plots_neg[[i]]=
    ggplot(expr_data, aes(x=age, y=expression)) + # do not set color=sex because some ages have only 1 female
    ...
    geom_smooth(method="loess", show.legend=FALSE, linewidth=0.5, fill="gray93", alpha=0.5, color="black") +
    ggpmisc::stat_correlation(vstep=0, show.legend=FALSE, size=3.5, method="spearman", ggpmisc::use_label(c("R", "p")), small.p=T)
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Rough_",c(1:length(celltypes_all)),"_pos.pdf")))
...
for (i in 1:length(celltypes_all)) {
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_CD8T.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Inter_",c(1:length(celltypes_all)),"_pos.pdf")))
...
for (i in 1:length(celltypes_all)) {
  ...
  ### Save all
  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",i,"_neg.pdf"), height=ceiling(length(Cor_df_neg)/3)*2, width=9)
  plot(cor_plots_neg[[i]])
  dev.off()

  pdf(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",i,"_pos.pdf"), height=ceiling(length(Cor_df_pos)/3)*2, width=9)
  plot(cor_plots_pos[[i]])
  dev.off()
}
qpdf::pdf_combine(input=c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_neg.pdf"),
                          paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_pos.pdf")),
                  output="~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_CD8T.pdf")
unlink(c(paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_neg.pdf"),
         paste0("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_CorPlot_Detailed_",c(1:length(celltypes_all)),"_pos.pdf")))
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Bulkanalysis_RNAexpr.Cor.wAges_Volcano_Rough_MarkedwKnownGenes.pdf", height=5, width=6)
