### Process Japan supercentenarians dataset
#####################################
###

###
japan.data=data.table::fread("~/Project_PBMCage/Japan supercentenarians/01.UMI.txt.gz")
japan.metadata=data.table::fread("~/Project_PBMCage/Japan supercentenarians/03.Cell.Barcodes.txt.gz", header=FALSE)
japan.data=as.data.frame(japan.data)
rownames(japan.data)=japan.data$V1
japan.data=japan.data[,2:ncol(japan.data)]
# convert to gene symbol
if (F) { # make a mapping dictionary from https://ftp.ensembl.org/pub/current_gtf/homo_sapiens/Homo_sapiens.GRCh38.110.gtf.gz
  MyAnnotTable=data.table::fread("~/Project_PBMCage/Homo_sapiens.gene_info.gz")
  head(MyAnnotTable)
  View(MyAnnotTable)
  ENSEMBL_ID=SYMBOL=list()
  processbar=txtProgressBar(min=0, max=length(MyAnnotTable$V9), style=3, width=60, char="=")
  for (x in 1:length(MyAnnotTable$V9)) {
    ENSEMBL_ID[[x]]=strsplit(MyAnnotTable$V9[x], ";")[[1]][grepl("gene_id",strsplit(MyAnnotTable$V9[x], ";")[[1]])]
    SYMBOL[[x]]=strsplit(MyAnnotTable$V9[x], ";")[[1]][grepl("gene_name",strsplit(MyAnnotTable$V9[x], ";")[[1]])]
    setTxtProgressBar(processbar, x)
  }
  close(processbar)
  SYMBOL[sapply(SYMBOL, function(x) length(x)==0L)]=NA
  MyAnnotTable_extracted=data.frame(GeneENSEMBL=unlist(ENSEMBL_ID), GeneSYMBOL=unlist(SYMBOL))
  MyAnnotTable_extracted=MyAnnotTable_extracted %>%
    mutate(GeneENSEMBL_clean=gsub("gene_id |\"","",GeneENSEMBL)) %>%
    mutate(GeneSYMBOL_clean=gsub(" gene_name |\"","",GeneSYMBOL))
  MyAnnotTable_extracted=MyAnnotTable_extracted[(!duplicated(MyAnnotTable_extracted$GeneENSEMBL_clean)) & (!duplicated(MyAnnotTable_extracted$GeneSYMBOL_clean)),]
  write.table(MyAnnotTable_extracted, "~/Project_PBMCage/Homo_sapiens.GRCh38.110.gtf.gz_arranged.txt", sep="\t")
}

if (F) { # if using ENSEMBL to check percent.mt, then:
  percentMT_genes=c("ENSG00000198888","ENSG00000198763","ENSG00000198840","ENSG00000212907",
                    "ENSG00000198886","ENSG00000198786","ENSG00000198695","ENSG00000198727",
                    "ENSG00000198804","ENSG00000198712","ENSG00000198938","ENSG00000198899",
                    "ENSG00000228253","ENSG00000210082","ENSG00000211459","ENSG00000210127",
                    "ENSG00000210174","ENSG00000210135","ENSG00000210154","ENSG00000210140",
                    "ENSG00000210194","ENSG00000210107","ENSG00000210164","ENSG00000210176",
                    "ENSG00000210100","ENSG00000209082","ENSG00000210191","ENSG00000210156",
                    "ENSG00000210112","ENSG00000210049","ENSG00000210196","ENSG00000210151",
                    "ENSG00000210184","ENSG00000210195","ENSG00000210117","ENSG00000210144","ENSG00000210077")
}

table(rownames(japan.data)==MyAnnotTable_extracted$GeneENSEMBL_clean[match(rownames(japan.data), MyAnnotTable_extracted$GeneENSEMBL_clean)]) # check
japan.data_toConvert=japan.data
japan.data_toConvert$ENSEMBL=rownames(japan.data_toConvert)
japan.data_toConvert$SYMBOL=MyAnnotTable_extracted$GeneSYMBOL_clean[match(rownames(japan.data), MyAnnotTable_extracted$GeneENSEMBL_clean)]
japan.data_toConvert=na.omit(japan.data_toConvert)
dim(japan.data); dim(japan.data_toConvert) # check
rownames(japan.data_toConvert)=japan.data_toConvert$SYMBOL
japan.data_toConvert=japan.data_toConvert[, 1:(ncol(japan.data_toConvert)-2)]

# add metadata
japan.metadata=as.data.frame(japan.metadata)
rownames(japan.metadata)=japan.metadata$V1
colnames(japan.metadata)=c("V1","donor_id","V3")

metadata=data.frame(donor_id=c("SC1","SC2","SC3","SC4","SC5","SC6","SC7","CT1","CT2","CT3","CT4","CT5"),
                    age=c(110,110,110,110,110,110,110,50,70,60,70,80),
                    sex=c("F","M","F","F","F","F","M","M","F","F","F","M"))
japan.metadata_updated=merge(japan.metadata, metadata, merge="donor_id")
rownames(japan.metadata_updated)=japan.metadata_updated$V1
japan.metadata_updated=japan.metadata_updated[,c(1,4,5)]
dim(japan.data_toConvert); dim(japan.metadata_updated) # check
japan.metadata_updated=japan.metadata_updated[match(colnames(japan.data_toConvert), rownames(japan.metadata_updated)),]
table(rownames(japan.metadata_updated)==colnames(japan.data_toConvert)) # check
japan.metadata_updated$dataset="Japan_sc"

# create the seurat obj
library(Seurat)
pbmc.japan=CreateSeuratObject(counts=japan.data_toConvert, meta.data=japan.metadata_updated)

# add percent.mt and percent.rb
pbmc.japan[["percent.rb"]]=PercentageFeatureSet(pbmc.japan, pattern="^RP[SL]")
pbmc.japan[["percent.mt"]]=PercentageFeatureSet(pbmc.japan, pattern="^MT-")

# QC
VlnPlot(pbmc.japan, features = c("nFeature_RNA","nCount_RNA","percent.mt"), ncol=3)
saveRDS(pbmc.japan, "~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc.rds")
pbmc.japan_filtered=subset(pbmc.japan, nFeature_RNA>200 & nFeature_RNA<2500 & percent.mt<10)
saveRDS(pbmc.japan_filtered, "~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_filtered.rds")

# ### Save seurt4 obj of pbmc.japan
# pbmc.japan_filtered=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_filtered.rds")
# query_counts=LayerData(pbmc.japan_filtered, assay="RNA", layer="counts", fast=TRUE)
# dimnames(query_counts)=list(rownames(pbmc.japan_filtered@assays$RNA@features), rownames(pbmc.japan_filtered@assays$RNA@cells))
# query_meta=pbmc.japan_filtered[[]]
# save(query_counts, query_meta, file="~/Project_PBMCage/Japan supercentenarians/FORscPred_Elements.RData")

# try merge without integration
pbmc.japan_filtered=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_filtered.rds")
pbmc.japan_filtered_try=pbmc.japan_filtered %>%
  NormalizeData() %>%
...
DimPlot(pbmc.japan_filtered_try, group.by=c("sex", "donor_id", "seurat_clusters"))

# have to integrate
pbmc.japan_filtered_try[["RNA"]]=split(pbmc.japan_filtered[["RNA"]], f=pbmc.japan_filtered$donor_id)
pbmc.japan_filtered_try
pbmc.japan_filtered_try=pbmc.japan_filtered_try %>%
  NormalizeData() %>%
  FindVariableFeatures() %>%
  ScaleData() %>%
  RunPCA() %>%
  FindNeighbors(dims=1:30) %>%
  FindClusters(resolution=0.5)
pbmc.japan_filtered_try2=IntegrateLayers(pbmc.japan_filtered_try, method=RPCAIntegration, orig.reduction="pca", new.reduction="integrated.rpca")
pbmc.japan_filtered_try3=pbmc.japan_filtered_try2 %>%
...
DimPlot(pbmc.japan_filtered_try3, reduction="umap.rpca", group.by=c("sex", "donor_id"), label.size=2)
saveRDS(pbmc.japan_filtered_try3, "~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_INTEGRATED.rds")

### Annotate the cells
pbmc.japan_filtered_try3=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_INTEGRATED.rds")
pbmc.seu_merged=JoinLayers(pbmc.japan_filtered_try3)
...
View(top10)
markers_2=FindMarkers(pbmc.seu_merged, ident.1="6", ident.2="9")
top10_2=markers_2 %>%
  slice_max(n=20, order_by=avg_log2FC)
View(top10_2)
#0: NK.CD56dim
...
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(3,4), "CD4T cells", seurat_clusters)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(1,6), "CD8T cells", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(7,10,15), "B cells", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(11,14), "DCs", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(2,9), "Monocytes", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(0,5), "NK cells", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(8,12), "Other cells", temptannot)) %>%
  mutate(temptannot=ifelse(seurat_clusters %in% c(13), "OtherT", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
table(rownames(tempt_annot_df)==colnames(pbmc.japan_filtered_try)) # check
...
B_subset_try=FindClusters2(B_subset, cluster.range=c(5,6), by=0.1, res=0.2, verbose=T)
...
#1: B.mem
#2: B.pc
#3: B.naive.lin_infidel.OtherT.NKT
#4: B.naive.lin_infidel.Other.Plt

highlighted=WhichCells(B_subset_try, idents=4)
...
tempt_annot_df=tempt_annot_df %>%
  ...
  mutate(temptannot=ifelse(cell %in% WhichCells(B_subset_try, idents=1), "B.inter.IGHM", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(B_subset_try, idents=2), "B.pc", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(B_subset_try, idents=3), "B.naive.lin_infidel.OtherT.NKT", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(B_subset_try, idents=4), "B.naive.lin_infidel.Other.Plt", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
CD4_subset_try=FindClusters2(CD4_subset, cluster.range=c(5,6), by=0.1, res=0.3, verbose=T)
...
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=2, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD4_subset_try, idents=0), "CD4T.Tcm", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD4_subset_try, idents=1), "CD4T.Tcm.COTL1", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD4_subset_try, idents=2), "CD4T.Tcm.HNRNPH1", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD4_subset_try, idents=3), "CD4T.Tcm.HLA", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD4_subset_try, idents=4), "CD4T.CTL", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
CD8_subset_try=FindClusters2(CD8_subset, cluster.range=c(4,7), by=0.1, res=0.2, verbose=T)
...
#0: CD8T.Tem.GZMB_HNRNPH1
#1: CD8T.Tem.GZMB_FCGR3A
#2: CD8T.Tem.GZMKhi
#3: CD8T.naive.LINC02446

highlighted=WhichCells(CD8_subset_try, idents=3)
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=3, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD8_subset_try, idents=0), "CD8T.Tem.GZMB_HNRNPH1", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD8_subset_try, idents=1), "CD8T.Tem.GZMB_FCGR3A", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD8_subset_try, idents=2), "CD8T.Tem.GZMKhi", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(CD8_subset_try, idents=3), "CD8T.naive.LINC02446", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
DCs_subset_try=FindClusters2(DCs_subset, cluster.range=c(4,5), by=0.1, res=0.2, verbose=T)
...
#2: pDC
#3: ASDC_mDC
highlighted=WhichCells(DCs_subset_try, idents=3)
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=2, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(DCs_subset_try, idents=0), "DC.cDC2_2", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(DCs_subset_try, idents=1), "DC.mDC", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(DCs_subset_try, idents=2), "DC.pDC", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(DCs_subset_try, idents=3), "DC.AXL_mDC", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
Monocytes_subset_try=FindClusters2(Monocytes_subset, cluster.range=c(4,5), by=0.1, res=0.2, verbose=T)
...
top10=markers %>%
  ...
  slice_max(n=20, order_by=avg_log2FC)
View(top10)
#0:Mono.classical.HLA
#1:Mono.classical
#2:Mono.nonclassical
#3:Mono.classical.lin_infidel.Other.Plt
...
tempt_annot_df=tempt_annot_df %>%
  ...
  mutate(temptannot=ifelse(cell %in% WhichCells(Monocytes_subset_try, idents=3), "Mono.classical.lin_infidel.Other.Plt", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
NK_subset_try=FindClusters2(NK_subset, cluster.range=c(4,5), by=0.1, res=0.25, verbose=T)
...
top10=markers %>%
  ...
  slice_max(n=20, order_by=avg_log2FC)
View(top10)
#0: NK.CD56dim.FCER1Glo_KLRC2neg
#1: OtherT.gdT
#2: NK.CD56dim.FCER1G
#3: NK.CD56hi.FCGR3A
#4: NK.CD56dim.ISG
highlighted=WhichCells(NK_subset_try, idents=2)
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=2, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(NK_subset_try, idents=0), "NK.CD56dim.FCER1Glo_KLRC2neg", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(NK_subset_try, idents=1), "OtherT.gdT", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(NK_subset_try, idents=2), "NK.CD56dim.FCER1G", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(NK_subset_try, idents=3), "NK.CD56hi.FCGR3A", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(NK_subset_try, idents=4), "NK.CD56dim.ISG", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
Othercells_subset_try=FindClusters2(Othercells_subset, cluster.range=c(2,3), by=0.1, res=0.1, verbose=T)
...
highlighted=WhichCells(Othercells_subset_try, idents=2)
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=2, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  ...
  mutate(temptannot=ifelse(cell %in% WhichCells(Othercells_subset_try, idents=2), "Other.Plt", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")
...
OtherT_subset_try=FindClusters2(OtherT_subset, cluster.range=c(2), by=0.1, res=0.1, verbose=T)
...
DimPlot(pbmc.seu_merged, reduction="umap.rpca", label=T, label.size=2, repel=T, group.by="temptannot", cells.highlight=list(Cluster=highlighted), cols.highlight=c("darkred"), cols="gray80") + NoLegend()
tempt_annot_df=tempt_annot_df %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(OtherT_subset_try, idents=0), "OtherT.NKT.MKI67", temptannot)) %>%
  mutate(temptannot=ifelse(cell %in% WhichCells(OtherT_subset_try, idents=1), "Other.progenitor", temptannot))
table(tempt_annot_df$temptannot, useNA="ifany")

pbmc.seu_merged_tempt=AddMetaData(pbmc.seu_merged, metadata=tempt_annot_df$temptannot, col.name="tempt")
DimPlot(pbmc.seu_merged_tempt, reduction="umap.rpca", group.by="tempt", label=T, pt.size=0.2, label.size=2) + NoLegend()

### Add the metadata of celltype annotation
# as for so few cells, we assign all the lin_infidel and plt_contamin to their original celltypes
celltypes_detailed=unique(tempt_annot_df$temptannot)
tempt_annot_df=tempt_annot_df %>%
  mutate(Annot.inter=ifelse(temptannot %in% celltypes_detailed[grepl("B\\.inter",celltypes_detailed)], "B.inter", temptannot)) %>%
...
tempt_annot_df=tempt_annot_df %>%
  mutate(Annot.rough=ifelse(temptannot %in% celltypes_detailed[grepl("^B\\.",celltypes_detailed)], "B cells", temptannot)) %>%
...
pbmc.seu_merged_try=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_pbmc_INTEGRATED.rds")
...
saveRDS(pbmc.seu_merged, "~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")

# add the agecut metadata
AGECUT=list(c(50:80),c(110))
agecut_df=pbmc.seu_merged[[]]
agecut_df$age=as.factor(agecut_df$age)
agecut_df$agecut=agecut_df$age
agecut_df=agecut_df %>%
  mutate(agecut=ifelse(age %in% c(50:80), "CT", agecut)) %>%
  mutate(agecut=ifelse(age %in% c(110), "SC", agecut))
...
levels(pbmc.seu_merged$agecut)=c("CT","SC")
saveRDS(pbmc.seu_merged, "~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")
...
###
japan_sc=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")
japan_sc[[]] # percent.rb and percent.mt has been added
japan_sc # make sure the seurat_obj has been normalize, HVGs found, and scaled

### Cellcycle scoring
japan_sc=CellCycleScoring(japan_sc, 
                          s.features=cc.genes.updated.2019$s.genes, 
                          g2m.features=cc.genes.updated.2019$g2m.genes,
                          set.ident=TRUE)

### Save the mt, rb, and cell-cycle info
meta_info=japan_sc@meta.data %>% select(any_of(c("nCount_RNA","nFeature_RNA","dataset","donor_id","sex","age",
                                                 "percent.rb","percent.mt","Annot.detailed","Annot.inter","Annot.rough",
                                                 "agecut",
                                                 "S.Score","G2M.Score","Phase"))) %>%
  mutate(cellid=rownames(.))
data.table::fwrite(meta_info, "~/Project_PBMCage/Results/JapanSC_results/Key_meta_info.csv.gz", sep="\t")
...
plot_mt_all=
  ...
  theme_classic() +
  labs(x="age", y="Percent of mitochondrial genes (%)", title="SC cohort") +
...
plot_rb_all=
  ...
  geom_point(alpha=0.25) +
  theme_classic() +
  labs(x="age", y="Percent of ribosomal genes (%)", title="SC cohort") +
  theme(axis.text.x=element_text(size=8),
        axis.text.y=element_text(size=8),
        axis.title.x=element_text(size=9),
        axis.title.y=element_text(size=9),
...
plot_phase_all=
  ...
  facet_wrap(~Phase) +
  theme_classic() +
  labs(x="age", y="Percent of cell phase (%)", title="SC cohort") +
  theme(axis.text.x=element_text(size=8),
        axis.text.y=element_text(size=8),
        axis.title.x=element_text(size=9),
        axis.title.y=element_text(size=9),
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Mt.Rb.Cellcycle1.pdf", height=4.5, width=2.5)
# plot(plot_mt_all)
# plot(plot_mt_sep)
plot(plot_rb_all)
# plot(plot_rb_sep)
...
library(dplyr)
library(Seurat)
Seurat_obj=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")
...
saveRDS(pbmc.merged, "~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_rough.rds")
# -- add sex metadata
pbmc.merged=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_rough.rds")
...
pbmc.merged=merge(PseudoList[[1]], PseudoList[-1], add.cell.ids=celltypes, merge.data=FALSE, project="PBMCage_pseudobulk")
pbmc.merged=JoinLayers(pbmc.merged)
saveRDS(pbmc.merged, "~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_inter.rds")
# -- add celltypes metadata
pbmc.merged=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_inter.rds")
tempt_annot_df=pbmc.merged[[]]
...
pbmc.merged=merge(PseudoList[[1]], PseudoList[-1], add.cell.ids=celltypes, merge.data=FALSE, project="PBMCage_pseudobulk")
pbmc.merged=JoinLayers(pbmc.merged)
saveRDS(pbmc.merged, "~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_detailed.rds")
# -- add celltypes metadata
pbmc.merged=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated_Pseudobulk_detailed.rds")
...
###
pbmc.seu_merged=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")
...
pdf("~/Project_PBMCage/Plots/JapanSC_Proportion_vs._age.pdf", width=4, height=15)
...
pdf("~/Project_PBMCage/Plots/JapanSC_Proportion_vs._agecut.pdf", width=4, height=15)
for (i in 1:length(plistcowplot)) plot(plistcowplot[[i]])
dev.off()
...
pdf("~/Project_PBMCage/Plots/JapanSC_Counts_vs._age.pdf", width=4, height=15)
for (i in 1:length(plistcowplot)) plot(plistcowplot[[i]])
dev.off()
...
pdf("~/Project_PBMCage/Plots/JapanSC_Counts_vs._agecut.pdf", width=4, height=15)
for (i in 1:length(plistcowplot)) plot(plistcowplot[[i]])
dev.off()
...
write.table(COR_combined, "~/Project_PBMCage/Results/JapanSC_results/Cell_frequency_correlation.txt", sep="\t")
write.table(meta_info_combined, "~/Project_PBMCage/Results/JapanSC_results/Cell_frequency_values.txt", sep="\t")
...
### Plot the cellratio
object=readRDS("~/Project_PBMCage/Japan supercentenarians/JapanSC_annotated.rds")
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Cell_frequency_rough.pdf", width=6, height=3)
plot(plot_rough)
dev.off()

### Plot CD4/CD8 ratio
meta_info_combined=read.delim("~/Project_PBMCage/Results/JapanSC_results/Cell_frequency_values.txt", sep="\t")
...
meta_info_combined_CD4.CD8$agecut=fct_relevel(meta_info_combined_CD4.CD8$agecut, unique(meta_info_combined_CD4.CD8$agecut))
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Cell_frequency_rough.CD4_CD8_ratio.pdf", width=6, height=3)
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Cell_frequency_inter.pdf", width=15, height=6)
plot(plot_inter)
dev.off()
...
COR_combined=read.delim("~/Project_PBMCage/Results/JapanSC_results/Cell_frequency_correlation.txt", sep="\t")
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/Cell_frequency_detailed.pdf", width=15, height=6)
...
### Score CD8T and NKT
CD8T.NKT_detailedcelltypes=names(table(object$Annot.detailed))[grepl("CD8T\\.|NKT[^(lin_)]", names(table(object$Annot.detailed)))]
...
saveRDS(score_df,"~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/FunctionScore_CD8TandNKT.rds")

### Score NK
NK_detailedcelltypes=names(table(object$Annot.detailed))[grepl("NK\\.[^(lin_)]", names(table(object$Annot.detailed)))]
...
score_df$agecut=as.factor(score_df$agecut)
levels(score_df$agecut)=names(table(score_df$agecut))
saveRDS(score_df,"~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/FunctionScore_NK.rds")
...
### Plot
# NK
NK_df=readRDS("~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/FunctionScore_NK.rds")
...
# CD8T and NKT
CD8T.NKT_df=readRDS("~/Project_PBMCage/Japan supercentenarians/Tempt_RDS/FunctionScore_CD8TandNKT.rds")
...
# pdf("~/Project_PBMCage/Plots/JapanSC_Plots/JapanSC_Scoring_CD8_and_NK_rowmeans.pdf", width=5, height=8)
...
pdf("~/Project_PBMCage/Plots/JapanSC_Plots/JapanSC_Scoring_CD8_and_NK_scale.pdf", width=5, height=8)
NK_scoring_plots_forAge
NK_scoring_plots_forAgeCuts
CD8T.NKT_scoring_plots_forAge
CD8T.NKT_scoring_plots_forAgeCuts
