...
table(df$Annot.inter, df$agecut_merged, useNA="ifany")
table(df$Annot.detailed, df$agecut_merged, useNA="ifany")
...
sds=slingshot(rd, df$Annot.detailed, start.clus="Mono.nonclassical")
...
ggplot(df_merged, aes(x=pst)) +
  ...
  geom_density(aes(col=agecut_merged), fill="transparent", linewidth=0.5) +
  guides(col="none") +
