...
# remove NK.CD56hi.S100B as it has too few cells and cannot merge with other celltypes
CD8T=subset(CD8T, Annot.detailed!="NK.CD56hi.S100B")
...
meta_data=CD8T[[]]
meta_data=meta_data %>%
  mutate(Annot.detailed=ifelse(Annot.detailed %in% c("NK.CD56hi.CCL3","NK.CD56hi.COTL1"), "NK.CD56hi.COTL1", Annot.detailed))
...
df=df %>% mutate(agecut_merged=ifelse(age %in% c(19:30), "19~30",
...
saveRDS(df, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_Umap.rds")
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_Umap.rds")
...
sds=slingshot(rd, df$Annot.detailed, start.clus="NK.prolif.MCM") # Automate starts at "NK.prolif.MCM", which is nice.
saveRDS(sds, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_sds_origin.rds")

df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_Umap.rds")
sds=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_sds_origin.rds")
top_res=topologyTest(sds=sds, conditions=df$agecut_merged)
saveRDS(top_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_topologyTest.rds")

top_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_topologyTest.rds")
knitr::kable(top_res)
# turns out to be not sharing common trajectories
...
saveRDS(sds_merged, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_sds_merged.rds")
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_Umap.rds")
sds_merged=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_sds_merged.rds")
prog_res=progressionTest(sds_merged, conditions=df$agecut_merged, global=TRUE, lineages=TRUE)
saveRDS(prog_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_progressionTest.rds")

prog_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_progressionTest.rds")
knitr::kable(prog_res)
# turn out to be no difference
...
if (top_res$p.value>0.05) {
  ...
  if (n_lineage>1) {
    ...
    knitr::kable(dif_res)
    saveRDS(dif_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_differentiationTest.rds")
  }
}
...
saveRDS(fate_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_fateSelectionTest.rds")

fate_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_NK.slim_fateSelectionTest.rds")
...
p=
  ...
  theme_light() +
  labs(x="UMAP_1", y="UMAP_2", title="NK cells",
...
p_withLabel=
  ...
  ggrepel::geom_label_repel(data=midpoint %>% group_by(Annot.detailed) %>% summarize_at(c("umap_1","umap_2"), mean),
                            ...
                            aes(x=umap_1, y=umap_2, label=stringr::str_wrap(gsub("NK\\.","",gsub("_|\\."," ",Annot.detailed)), 15)),
                            alpha=0.8, size=3, label.padding=0.2, lineheight=1,
                            color="black") +
  guides(color=guide_legend(title="age"))

p_withLabel

pdf("~/Project_PBMCage/Plots/PBMCage_Plots/Pseudotime_Trajectory_NK.pdf", height=3.5, width=8)
plot(p_withLabel)
dev.off()
