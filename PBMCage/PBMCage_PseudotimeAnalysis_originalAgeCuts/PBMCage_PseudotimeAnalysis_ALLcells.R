...
# remove these two as they have too few cells and cannot merge with other cell types
celltype.remain=unique(THEObj$Annot.inter)[!unique(THEObj$Annot.inter) %in% 
                                             c("Other.Eryth","Other.Plt","Other.ILC","Other.Mast")] # ,"B.pbpc","NK.prolif","CD4T.prolif","CD8T.CTL"
CD8T=subset(THEObj, Annot.inter %in% celltype.remain)
# Seurat::DimPlot(CD8T, group.by="Annot.inter", label=T) + Seurat::NoLegend()
...
meta_data=CD8T[[]]
meta_data=meta_data %>%
  mutate(Annot.inter=ifelse(Annot.rough=="B cells", "B", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.rough=="CD4T cells", "CD4T", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.inter %in% c("CD8T.naive","CD8T.Tcm","CD8T.Tem","OtherT.gdT"), "CD8T/gdT", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.rough=="DCs", "DC", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.rough=="Monocytes", "Mono", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.inter %in% c("NK.CD56dim","NK.CD56hi","OtherT.NKT"), "NK/NKT", Annot.inter)) %>%
  mutate(Annot.inter=ifelse(Annot.inter %in% c("NK.prolif","CD4T.prolif","CD8T.CTL"), "cyclingT/NK", Annot.inter))

table(meta_data$Annot.inter, meta_data$agecut)

CD8T$Annot.inter=meta_data$Annot.inter
...
saveRDS(df, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_Umap.rds")
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_Umap.rds")
...
sds=slingshot(rd, df$Annot.inter, start.clus="Other.progenitor")
saveRDS(sds, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_sds_origin.rds")

df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_Umap.rds")
sds=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_sds_origin.rds")
top_res=topologyTest(sds=sds, conditions=df$agecut_merged)
saveRDS(top_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_topologyTest.rds")

top_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_topologyTest.rds")
knitr::kable(top_res)
  # |method     |thresh | statistic| p.value|
  # |:----------|:------|---------:|-------:|
  # |Classifier |0.01   | 0.9379667|       0|
...
saveRDS(sds_merged, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_sds_merged.rds")
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_Umap.rds")
sds_merged=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_sds_merged.rds")
prog_res=progressionTest(sds_merged, conditions=df$agecut_merged, global=TRUE, lineages=TRUE)
saveRDS(prog_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_progressionTest.rds")

prog_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_progressionTest.rds")
knitr::kable(prog_res)
  # |lineage | statistic|   p.value|
  # |:-------|---------:|---------:|
  # |All     | 0.2033333| 0.2870609|
  # |1       | 0.1888669| 0.8774971|
  # |2       | 0.1987302| 0.5392974|
  # |3       | 0.1900000| 0.7631097|
  # |4       | 0.2093548| 0.2487585|
  # |5       | 0.1975472| 0.5494442|
# turns out to be with no difference
...
if (top_res$p.value>0.05) {
  ...
  if (n_lineage>1) {
    ...
    knitr::kable(dif_res)
    saveRDS(dif_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_differentiationTest.rds")
  }
}
...
fate_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_ALLcells.slim_fateSelectionTest.rds")
knitr::kable(fate_res)
  # |pair | statistic|   p.value|
  # |:----|---------:|---------:|
  # |All  | 0.1847619| 0.9932376|
  # |1vs2 | 0.1930132| 0.8150512|
  # |1vs3 | 0.1860526| 0.9514040|
  # |1vs4 | 0.1896000| 0.8994202|
  # |1vs5 | 0.1880080| 0.9368410|
  # |2vs3 | 0.1771698| 0.9829778|
  # |2vs4 | 0.1895000| 0.8744815|
  # |2vs5 | 0.1909950| 0.8489685|
  # |3vs4 | 0.2030841| 0.3872321|
  # |3vs5 | 0.1992593| 0.5346647|
  # |4vs5 | 0.1860784| 0.9162232|
...
p=
  ...
  ggrastr::geom_point_rast(size=0.01, shape=20,
                           color=paletteer::paletteer_d("ggsci::category20_d3")[9]) +
  ...
  theme_light() +
  labs(x="UMAP_1", y="UMAP_2", title="PBMC",
...
midpoint=df %>% group_by(Annot.inter, agecut_merged) %>%
...
for (batch in unique(df$agecut_merged)) {
  ...
  edges_batch <- left_join(edges, cl_batch, by = c("from"="Annot.inter")) %>%
    left_join(cl_batch %>%
...
p_withLabel=
  p +
  ggrepel::geom_label_repel(data=midpoint %>% group_by(Annot.inter) %>% summarize_at(c("umap_1","umap_2"), mean),
                            inherit.aes=FALSE, force_pull=100,
                            aes(x=umap_1, y=umap_2, label=Annot.inter),
                            alpha=0.8,
                            color="black") +
...
pdf("~/Project_PBMCage/Plots/PBMCage_Plots/Pseudotime_Trajectory_ALLcells.pdf", height=5, width=7)
plot(p_withLabel)
dev.off()
