...
# df=df %>% 
#   mutate(agecut_merged=ifelse(agecut %in% c("19~24","25~30","31~42","43~51","52~58","59~64"), "19~64", "65~97"))
table(df$Annot.inter, df$agecut_merged)
table(df$Annot.detailed, df$agecut_merged)
saveRDS(df, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_Umap.rds")
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_Umap.rds")
...
sds=slingshot(rd, df$Annot.detailed)
saveRDS(sds, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_sds_origin.rds")

top_res=topologyTest(sds=sds, conditions=df$agecut_merged)
saveRDS(top_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_topologyTest.rds")

top_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_topologyTest.rds")
knitr::kable(top_res)
  # |method     |thresh | statistic| p.value|
  # |:----------|:------|---------:|-------:|
  # |Classifier |0.01   | 0.5374558|       0|
# turns out to be uncommon trajectories
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_Umap.rds")
sds=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_sds_origin.rds")
top_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_topologyTest.rds")
...
if (top_res$p.value<0.05) {
  ...
  saveRDS(sds_merged, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_sds_merged.rds")
}
...
df=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_Umap.rds")
sds_merged=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_sds_merged.rds")
prog_res=progressionTest(sds_merged, conditions=df$agecut_merged, global=TRUE, lineages=TRUE)
saveRDS(prog_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_progressionTest.rds")

prog_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_progressionTest.rds")
knitr::kable(prog_res)
  # |lineage | statistic|   p.value|
  # |:-------|---------:|---------:|
  # |All     | 0.1919900| 0.8679282|
  # |1       | 0.2038504| 0.3491976|
  # |2       | 0.1959590| 0.6925132|
...
if (top_res$p.value>0.05) {
  ...
  if (n_lineage>1) {
    ...
    knitr::kable(dif_res)
    saveRDS(dif_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_differentiationTest.rds")
  }
}
...
saveRDS(fate_res, "~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_fateSelectionTest.rds")

fate_res=readRDS("~/Project_PBMCage/Tempt_RDS/pseudotime_results/PBMCage_Mono_fateSelectionTest.rds")
knitr::kable(fate_res)
  # |pair | statistic|   p.value|
  # |:----|---------:|---------:|
  # |1vs2 | 0.2026036| 0.3648653|
...
p=
  ...
  theme_light() +
  labs(x="UMAP_1", y="UMAP_2", title="Monocytes",
...
# modify the names
midpoint_name_updated=midpoint %>%
  mutate(Annot.detailed=ifelse(Annot.detailed=="Mono.inter.C1Q","Mono.C1Q",
                               ifelse(Annot.detailed=="Mono.nonclassical.GNLY","Mono.GNLY",Annot.detailed)))
...
p_withLabel=
  p +
  ggrepel::geom_label_repel(data=midpoint_name_updated %>% group_by(Annot.detailed) %>% summarize_at(c("umap_1","umap_2"), mean),
...
pdf("~/Project_PBMCage/Plots/PBMCage_Plots/Pseudotime_Trajectory_Mono.pdf", height=5, width=7)
plot(p_withLabel)
dev.off()
...
# ##########################################################################
# ###### Below are for the case with common trajectory, inapplicable for Monocytes here
# ##########################################################################
# 
# 
